    ...
    def train(self, x_train, y_train, epoch, learning_rate, batch_size):        # mini-batch GD  
        ...
        for i in range(1, epoch +1):
            ...
            for j in range(0, len(x_train), batch_size):
                ...
                weights, biases = self.model.update_parameters(dW, dB, learning_rate)
        ...
        return (weights, biases, self.loss_list, self.accuracy_list)

    def plot(self):
        _, ax = plt.subplots(1, 1, figsize=(8, 6))
        ax.plot(self.accuracy_list, label="Accuracy", color="blue")
        ax.plot(self.loss_list, label="Loss", color="red")
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Value")
        ax.legend()
        ax.grid(True)
